...
/// A discrete point of contact between two objects
#[derive(Copy, Clone, Debug)]
pub struct DiscreteContact {
    /// Contact point for collider in global coordinates.
    pub a: Point3<f32>,
    /// Contact point for collidee in global coordinates.
    pub b: Point3<f32>,
    /// Collision normal on the surface of the collider.
    pub n: Vector3<f32>,
}

pub trait DiscreteContacts<RHS> {
    fn contact(&self, rhs: &RHS) -> Option<DiscreteContact>;
}

impl<ShapeA, ShapeB> DiscreteContacts<ShapeB> for ShapeA
where
    ShapeA: Convex + Volumetric,
    ShapeB: Convex + Volumetric,
{
    fn contact(&self, rhs: &ShapeB) -> Option<DiscreteContact> {
        let d = Vector3::new(1.0, 0.0, 0.0);
        let diff = MinkowskiDiff {
            s1: self,
            s2: rhs,
        ...
        if mag2 > COLLISION_EPSILON {
            ...
            simp.compute_contact(self, rhs).into()
        }
    }
}
